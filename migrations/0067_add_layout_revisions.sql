-- Migration: 0067_add_layout_revisions
-- Description: Add 'layout' as an entity type in the revisions table for tracking layout changes
...
-- Step 1: Create a new table with the updated CHECK constraint (adding 'layout')
CREATE TABLE IF NOT EXISTS revisions_new (
  id TEXT PRIMARY KEY,
  site_id TEXT NOT NULL,
  entity_type TEXT NOT NULL CHECK (entity_type IN ('page', 'product', 'category', 'theme', 'site', 'component', 'layout')),
...
-- To rollback this migration, you would need to recreate the table without 'layout'
-- This should only be done if no layout revisions have been created yet.
-- 1. ALTER TABLE revisions RENAME TO revisions_old;
-- 2. CREATE TABLE revisions (...with original CHECK constraint without 'layout'...);
-- 3. INSERT INTO revisions SELECT * FROM revisions_old WHERE entity_type != 'layout';
-- 4. DROP TABLE revisions_old;
